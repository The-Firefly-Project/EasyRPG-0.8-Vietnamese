...
void Player::CreateGameObjects() {
	...
	if (!game_config.patch_override) {
		...
		if (!FileFinder::Game().FindFile("dynloader.dll").empty()) {
			...
			Output::Warning("Trò chơi này sử dụng DynRPG và sẽ không chạy đúng cách.");
		}
...
bool Player::ChangeResolution(int width, int height) {
	if (!DisplayUi->ChangeDisplaySurfaceResolution(width, height)) {
		Output::Warning("Thay đổi độ phân giải thành {}x{} thất bại", width, height);
		return false;
	}
...
void Player::RestoreBaseResolution() {
	...
	if (!Player::ChangeResolution(SCREEN_TARGET_WIDTH, SCREEN_TARGET_HEIGHT)) {
		...
		Output::Error("Không thể khôi phục độ phân giải cơ sở");
	}
}
...
void Player::GuessNonStandardExtensions() {
	...
	if (!FileFinder::IsRPG2kProject(FileFinder::Game()) &&
		...
		if (rpg2kRemap.Empty()) {
			...
			Output::Error("{}\n\n{}\n\n{}\n\n{}","Không tìm thấy trò chơi hợp lệ.",
				"EasyRPG phải được chạy từ thư mục trò chơi chứa\nRPG_RT.ldb and RPG_RT.lmt.",
				"Phần mềm này chỉ hỗ trợ các trò chơi RPG Maker 2000\nvà 2003.",
...
void Player::LoadDatabase() {
	...
	if (is_easyrpg_project) {
		...
		if (!edb_stream) {
			Output::Error("Lỗi khi tải {}", DATABASE_NAME_EASYRPG);
			return;
		}
		...
		if (!emt_stream) {
			Output::Error("Lỗi khi tải {}", TREEMAP_NAME_EASYRPG);
			return;
		}
		...
		if (!ldb_stream) {
			Output::Error("Lỗi khi tải {}", ldb_name);
			return;
		}
		...
		if (!lmt_stream) {
			Output::Error("Lỗi khi tải {}", lmt_name);
			return;
		}
...
void Player::LoadSavegame(const std::string& save_name, int save_id) {
	...
	if (!save_stream) {
		Output::Error("Lỗi khi tải {}", save_name);
		return;
	}
	...
	if (ver > PLAYER_SAVEGAME_VERSION) {
		Output::Warning("Trò chơi lưu trữ này được tạo bằng {}, phiên bản mới hơn phiên bản hiện tại của EasyRPG Player ({})",
...
void Player::SetupBattleTest() {
	...
	if (troop == nullptr) {
		Output::Error("BattleTest: ID nhóm quái vật {} không hợp lệ", args.troop_id);
	}
