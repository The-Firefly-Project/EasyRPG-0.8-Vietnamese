...
void Scene::PopUntil(SceneType type) {
	...
	Output::Warning("Cảnh {} được yêu cầu không nằm trong danh sáhc", scene_names[type]);
	DEBUG_VALIDATE("PopUntil");
}
...
inline void Scene::DebugValidate(const char* caller) {
	...
	for (auto& scene: instances) {
		if (present[scene->type]) {
			...
			Output::Error("Nhiều cảnh có type={} đang nằm trong danh sách của mảng Scene!", scene_names[scene->type]);
		}
		present[scene->type] = true;
	}
	if (instances[0]->type != Null) {
		Output::Error("Scene.instances[0] có type={} đang nằm trong danh sách của mảng Scene!", scene_names[instances[0]->type]);
	}
}
