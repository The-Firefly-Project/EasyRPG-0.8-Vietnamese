...
void Game_Map::Setup(std::unique_ptr<lcf::rpg::Map> map_in) {
			...
			|| can_teleport == lcf::rpg::MapInfo::TriState_parent)
	{
		...
		if (parent_index == current_index) {
			Output::Warning("Bản đồ {} có bản đồ cha là chính nó!", current_index);
			break;
		}
		if (parent_index < 0) {
			Output::Warning("Bản đồ {} có ID bản đồ cha không hợp lệ {}!", lcf::Data::treemap.maps[current_index].ID, lcf::Data::treemap.maps[current_index].parent_map);
			break;
		}
...
std::unique_ptr<lcf::rpg::Map> Game_Map::loadMapFile(int map_id) {
	...
	if (map_file.empty()) {
		...
		if (map_file.empty()) {
			Output::Error("Tải bản đồ {} thất bại.\nKhông thể tìm thấy bản đồ này.", map_name);
			return nullptr;
		}
...
std::vector<int> Game_Map::GetEncountersAt(int x, int y) {
	...
	std::function<bool(int)> is_acceptable = [=](int troop_id) {
		...
		if (!troop) {
			Output::Warning("GetEncountersAt: ID quân đội {} trong danh sách chạm trán không hợp lệ", troop_id);
			return false;
		}
