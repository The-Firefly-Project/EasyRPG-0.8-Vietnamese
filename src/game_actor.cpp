...
bool Game_Actor::UseItem(int item_id, const Game_Battler* source) {
	...
	if (!item) {
		Output::Warning("UseItem: Không thể dùng vật phẩm {} không hợp lệ", item_id);
		return false;
	}
...
bool Game_Actor::LearnSkill(int skill_id, PendingMessage* pm) {
	if (skill_id > 0 && !IsSkillLearned(skill_id)) {
		const lcf::rpg::Skill* skill = lcf::ReaderUtil::GetElement(lcf::Data::skills, skill_id);
		if (!skill) {
			Output::Warning("Actor {}: Không thể học kĩ năng {} không hợp lệ", GetId(), skill_id);
			return false;
		}
...
void Game_Actor::ChangeBattleCommands(bool add, int id) {
	...
	if (add) {
		...
		if (!cmd) {
			Output::Warning("ChangeBattleCommands: Không thể thêm lệnh chiến đấu {} không hợp lệ", id);
			return;
		}
...
const std::vector<const lcf::rpg::BattleCommand*> Game_Actor::GetBattleCommands() const {
	...
	for (int command_index : obc) {
		...
		if (!cmd) {
			Output::Warning("GetBattleCommands: ID lệnh chiến đấu {} không hợp lệ", command_index);
			continue;
		}
		...
		)
{
	...
	if (new_class_id != 0 && cls == nullptr) {
		Output::Warning("Actor {}: Không thể thay đổi thành class {} không hợp lệ", GetId(), new_class_id);
		return;
	}
...
int Game_Actor::GetBattleAnimationId() const {
	...
	if (data.battler_animation <= 0) {
		...
		if ((data.class_id > 0) && GetClass()) {
			...
			if (!anima) {
				Output::Warning("Actor {}: ID hoạt ảnh chiến đấu {} không hợp lệ", GetId(), dbActor->battler_animation);
				return 0;
			}
...
void Game_Actor::RemoveInvalidData() {
	...
	*/
	...
	if (data.class_id > 0) {
		...
		if (!cls) {
			Output::Warning("Actor {}: Đang loại bỏ class {} không hợp lệ", GetId(), data.class_id);
	...
	for (int16_t skill_id : GetSkills()) {
		const lcf::rpg::Skill* skill = lcf::ReaderUtil::GetElement(lcf::Data::skills, skill_id);
		if (!skill) {
			Output::Warning("Actor {}: Đang loại bỏ class {} không hợp lệ", GetId(), skill_id);
	...
	if (GetStates().size() > lcf::Data::states.size()) {
		Output::Warning("Actor {}: Mảng trạng thái nhân vật đang chứa trạng thái không hợp lệ ({} > {})", GetId(), GetStates().size(), lcf::Data::states.size());
	...
	if (GetLevel() == 0) {
		...
		Output::Warning("Actor {}: Cấp độ {} không hợp lệ, đã thay đổi lại thành 1", GetId(), GetLevel());
		SetLevel(1);
	} else if (GetLevel() > GetMaxLevel()) {
		Output::Warning("Actor {}: Cấp độ {} không hợp lệ, đã thay đổi lại thành {}", GetId(), GetLevel(), GetMaxLevel());
