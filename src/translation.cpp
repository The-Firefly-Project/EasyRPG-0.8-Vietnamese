...
void Translation::InitTranslations()
{
	...
	for (const auto& tr_name : *game_tree) {
		if (tr_name.first == TRDIR_NAME_LEGACY) {
			Output::Warning("Cảnh báo tính năng không được sử dụng nữa: Vui lòng đổi tên thư mục \"languages\" thành \"language\"");
...
void Translation::SelectLanguage(StringView lang_id)
{
	...
	if (!lang_id.empty()) {
		...
		if (!root) {
			Output::Error("Không thể tải bản dịch. Thư mục 'Language' không tồn tại");
			return;
		}
		...
		if (language_tree) {
			...
			Output::Warning("Bản dịch cho ngôn ngữ '{}' không tồn tại", lang_id);
		}
	} else {
...
void Dictionary::FromPo(Dictionary& res, std::istream& in) {
	...
	auto extract_string = [&](size_t offset) -> std::string {
		if (offset >= line_view.size()) {
			Output::Error("Lỗi phân tích (dòng {}) trống", line_number);
			return "";
		}
		...
		for (char c : line_view.substr(offset)) {
			if (!first_quote) {
				...
				Output::Error(R"(Lỗi phân tích (dòng {}): " được mong muốn, nhưng lại nhận được "{}": {})", line_number, c, line);
				return "";
			}
			...
			if (!slash && c == '\\') {
				...
				switch (c) {
					...
					case '"':
						...
						break;
					default:
						Output::Error(R"(Lỗi phân tích (dòng {}): \, \n hoặc " được mong muốn, nhưng lại nhận được "{}": {})", line_number, c, line);
						break;
				}
			} else {
				// no-slash
		...
		Output::Error("Lỗi phân tích (dòng {}): Dòng không được kết thúc: {}", line_number, line);
		return out.str();
	};
