...
bool Remove(int state_id, StateVec& states, const PermanentStates& ps) {
	const lcf::rpg::State* state = lcf::ReaderUtil::GetElement(lcf::Data::states, state_id);
	if (!state) {
		Output::Warning("State::Remove: Không thể bỏ trạng thái với ID {} không hợp lệ", state_id);
		return false;
	}
...
bool RemoveAllBattle(StateVec& states, const PermanentStates& ps) {
	...
	for (int i = 0; i < (int)states.size(); ++i) {
		...
		if (state == nullptr) {
			Output::Warning("State::RemoveAllBattle: Không thể bỏ trạng thái với ID {} không hợp lệ", state_id);
			continue;
		}
...
lcf::rpg::State::Restriction GetSignificantRestriction(const StateVec& states) {
	...
	for (int i = 0; i < (int)states.size(); ++i) {
		...
		auto* state = lcf::ReaderUtil::GetElement(lcf::Data::states, state_id);
		if (state == nullptr) {
			Output::Warning("State::GetSignificantRestriction: Không thể bỏ trạng thái với ID {} không hợp lệ", state_id);
			continue;
		}
...
const lcf::rpg::State* GetSignificantState(const StateVec& states) {
	...
	for (int i = 0; i < (int)states.size(); ++i) {
		...
		auto* state = lcf::ReaderUtil::GetElement(lcf::Data::states, state_id);
		if (state == nullptr) {
			Output::Warning("State::GetSignificantState: Không thể bỏ trạng thái với ID {} không hợp lệ", state_id);
			continue;
		}
