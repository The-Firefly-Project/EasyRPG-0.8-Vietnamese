...
bool Game_Interpreter::OnFinishStackFrame() {
	...
	if (is_base_frame && event_id > 0) {
		...
		if (!evnt) {
			Output::Error("Lệnh gọi được hoàn thành với ID sự kiện {} không hợp lệ. Điều này có thể được gây ra bởi dịch chuyển tức thời của một phương tiện nào đó?", event_id);
		} else if (main_flag) {
...
bool Game_Interpreter::CommandControlVariables(lcf::rpg::EventCommand const& com) { // code 10220
	...
	if (EP_UNLIKELY(operand >= 9 && !Player::IsPatchManiac())) {
		Output::Warning("ControlVariables: Toán hạng {} không được hỗ trợ", operand);
		return true;
	}
	...
	if (target >= 0 && target <= 4) {
		...
		if (EP_UNLIKELY(operation >= 6 && !Player::IsPatchManiac())) {
			Output::Warning("ControlVariables: Toán hạng {} không được hỗ trợ", operation);
			return true;
		}
...
Game_Character* Game_Interpreter::GetCharacter(int event_id) const {
	if (event_id == Game_Character::CharThisEvent) {
		...
		if (event_id == 0) {
			// With no map parent
			Output::Warning("Không thể sử dụng ThisEvent trong sự kiện chung: Không được gọi từ sự kiện bản đồ");
			return nullptr;
		}
	}
...
bool Game_Interpreter::CommandSetVehicleLocation(lcf::rpg::EventCommand const& com) { // code 10850
	...
	if (!vehicle) {
		...
		if (vehicle_id == 0) {
			...
			Output::Warning("SetVehicleLocation: ID nhân vật {} không hợp lệ", vehicle_id);
			return true;
		}
	}
	...
	if (Main_Data::game_player->GetVehicle() == vehicle) {
		...
		if (!main_flag && event_id != 0) {
			Output::Error("VehicleTeleport không được phép từ sự kiện bản đồ song song! Id={}", event_id);
		}
...
bool Game_Interpreter::CommandShowPicture(lcf::rpg::EventCommand const& com) { // code 11110
	...
	if (pic_id <= 0) {
		Output::Error("ShowPicture: ID hình ảnh không hợp lệ ({}) được yêu cầu", pic_id);
	}
...
bool Game_Interpreter::CommandMovePicture(lcf::rpg::EventCommand const& com) { // code 11120
	...
	if (pic_id <= 0) {
		Output::Error("MovePicture: ID hình ảnh không hợp lệ ({}) được yêu cầu", pic_id);
	}
...
bool Game_Interpreter::CommandErasePicture(lcf::rpg::EventCommand const& com) { // code 11130
	...
	if (com.parameters.size() > 1 && Player::IsRPG2k3ECommands()) {
		...
		for (int i = pic_id; i <= pic_id_max; ++i) {
			if (i <= 0) {
				Output::Error("ErasePicture: ID hình ảnh không hợp lệ ({}) được yêu cầu", i);
			}
		...
		if (pic_id <= 0) {
			Output::Error("ErasePicture: ID hình ảnh không hợp lệ ({}) được yêu cầu", pic_id);
		}
...
bool Game_Interpreter::CommandKeyInputProc(lcf::rpg::EventCommand const& com) { // code 11610
	...
	if (is_maniac) {
		auto check_mouse = [&](auto idx) {
			...
			if (result) {
				Output::Warning("ManiacPatch: Chuột không được hỗ trợ trên nền tảng này");
				result = false;
			}
#endif
			return result;
...
bool Game_Interpreter::CommandConditionalBranch(lcf::rpg::EventCommand const& com) { // Code 12010
		...
		if (Player::IsPatchManiac()) {
			switch (com.parameters[1]) {
				case 0:
					...
					Output::Warning("Maniac: Điều kiện 'Loaded' vẫn chưa được triển khai");
					result = false;
					break;
				case 1:
	...
	case 14:
		...
		break;
	default:
		Output::Warning("ConditionalBranch: Nhánh {} không được hỗ trợ", com.parameters[0]);
	}
...
bool Game_Interpreter::CommandCallEvent(lcf::rpg::EventCommand const& com) { // code 12330
	...
	if (!page) {
		Output::Warning("CallEvent: Không thể gọi trang {} không tồn tại của sự kiện {}", event_page, evt_id);
		return true;
	}
...
bool Game_Interpreter::CommandChangeClass(lcf::rpg::EventCommand const& com) { // code 1008
	...
	if (!cls && class_id != 0) {
		Output::Warning("ChangeClass: Không thể thay đổi class. Class {} không hợp lệ", class_id);
		return true;
	}
...
bool Game_Interpreter::CommandManiacGetSaveInfo(lcf::rpg::EventCommand const& com) {
	...
	if (save_number <= 0) {
		Output::Debug("ManiacGetSaveInfo: Thứ tự bản lưu {} không hợp lệ", save_number);
		return true;
	}
	...
	if (!save_stream) {
		Output::Debug("ManiacGetSaveInfo: Không thể tìm thấy bản lưu {}", save_number);
		return true;
	}

	auto save = lcf::LSD_Reader::Load(save_stream, Player::encoding);
	if (!save) {
		Output::Debug("ManiacGetSaveInfo: bản lưu {} đã bị lỗi", save_number);
...
bool Game_Interpreter::CommandManiacSave(lcf::rpg::EventCommand const& com) {
	...
	if (slot <= 0) {
		Output::Debug("ManiacSave: Thứ tự bản lưu {} không hợp lệ", slot);
		return true;
	}
...
bool Game_Interpreter::CommandManiacLoad(lcf::rpg::EventCommand const& com) {
	...
	int slot = ValueOrVariable(com.parameters[0], com.parameters[1]);
	if (slot <= 0) {
		Output::Debug("ManiacLoad: Thứ tự bản lưu {} không hợp lệ", slot);
		return true;
	}
	...
	if (!save) {
		Output::Debug("ManiacLoad: Không thể tìm thấy bản lưu {}", slot);
		return true;
	}
...
bool Game_Interpreter::CommandManiacGetMousePosition(lcf::rpg::EventCommand const& com) {
	...
	if (!warned) {
		...
		Output::Warning("Maniac Patch: Chuột không được hỗ trợ trên nền tảng này");
		warned = true;
	}
	return true;
#endif
...
bool Game_Interpreter::CommandManiacSetMousePosition(lcf::rpg::EventCommand const&) {
	...
	Output::Warning("Maniac Patch: Lệnh SetMousePosition chưa được hỗ trợ");
	return true;
}
...
bool Game_Interpreter::CommandManiacShowStringPicture(lcf::rpg::EventCommand const& com) {
	...
	if (pic_id <= 0) {
		Output::Error("ShowStringPic: ID hình ảnh không hợp lệ ({}) được yêu cầu", pic_id);
		return true;
	}
	...
	if (params.width < 0 || params.height < 0) {
		Output::Warning("ShowStringPic: Kích thước cửa sổ không hợp lệ {}x{} (id={})", params.width, params.height, pic_id);
		return true;
	}
	...
	if (components.size() < 4) {
		Output::Warning("ShowStringPic: Văn bản không hợp lệ (id={})", pic_id);
		return true;
	}
...
bool Game_Interpreter::CommandManiacControlVarArray(lcf::rpg::EventCommand const& com) {
	...
	switch (op) {
		...
		case 15:
			...
			break;
		default:
			Output::Warning("ManiacControlVarArray: Hoạt động không xác định {}", op);
	}
...
bool Game_Interpreter::CommandManiacKeyInputProcEx(lcf::rpg::EventCommand const& com) {
	...
	Output::Warning("Maniac KeyInputProc: Bàn phím không được hỗ trợ trên nền tảng này");
	...
	if (operation == 0 || operation == 1) {
		...
		Output::Warning("Maniac KeyInputProcEx: Tay cầm không được hỗ trợ");
	}
...
bool Game_Interpreter::CommandManiacControlGlobalSave(lcf::rpg::EventCommand const& com) {
	...
	if (operation == 0 || operation == 1) {
		...
		if (!lgs_out) {
			Output::Warning("Maniac ControlGlobalSave: Lỗi khi lưu");
			return true;
		}
