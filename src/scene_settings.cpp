...
void Scene_Settings::CreateMainWindow() {
	...
	if (Scene::Find(Scene::Title)) {
		options.push_back("<Thoát trò chơi>");
	}
...
void Scene_Settings::CreateOptionsWindow() {
	...
	std::vector<std::string> input_mode_items = {"Thêm", "Xoá", "Cài đặt lại"};
	...
	input_mode_window->UpdateHelpFn = [](Window_Help& win, int index) {
		if (index == 0) {
			win.SetText("Thêm một tổ hợp phím mới");
		} else if (index == 1) {
			win.SetText("Xoá một tổ hợp phím");
		} else if (index == 2) {
			win.SetText("Đặt lại tổ hợp phím về mặc định");
		}
	};
...
void Scene_Settings::SetMode(Window_Settings::UiMode new_mode) {
	...
	switch (mode) {
		...
		case Window_Settings::eInputButtonAdd:
			...
			input_mode_window->SetVisible(true);
			input_help_window->SetVisible(true);
			input_help_window->SetText("Nhấn một phím để thêm vào tổ hợp. Để hủy bỏ tổ hợp, hãy đợi 3 giây");
			break;
		case Window_Settings::eInputButtonRemove:
			...
			input_mode_window->SetVisible(true);
			input_help_window->SetVisible(true);
			input_help_window->SetText("Chọn một phím mà bạn muốn loại bỏ");
			break;
...
bool Scene_Settings::RefreshInputEmergencyReset() {
	if (Input::GetAllRawPressed().count() >= 4) {
		if (input_reset_counter == 0) {
			Output::InfoStr("Bắt đầu đặt lại phím khẩn cấp");
			Output::InfoStr("Giữ các phím trong vòng 3 giây");
		}
		input_reset_counter++;
		...
		if (input_reset_counter == Game_Clock::GetTargetGameFps() * 3) {
			if (input_window->GetInputButton() == Input::InputButton::BUTTON_COUNT) {
				...
				Output::InfoStr("Tất cả các phím đã được đặt lại về mặc định");
				if (input_window->GetActive()) {
					input_window->SetIndex(0);
				}
				Input::ResetAllMappings();
			} else {
				Output::Info("Phím {} đã được đặt lại về mặc định", Input::kButtonNames.tag(input_window->GetInputButton()));
				Output::Info("Để đặt lại các phím nhấn giữ thêm 3 giây nữa");
				if (input_window->GetActive()) {
					input_window->SetIndex(0);
				}
				input_window->ResetMapping();
		...
		if (input_reset_counter > 0) {
			Output::InfoStr("Kết thúc đặt lại phím khẩn cấp");
...
bool Scene_Settings::SaveConfig(bool silent) {
	...
	if (!cfg_out) {
		if (silent) {
			Output::Debug("Lưu tệp tin cài đặt thất bại!");
		} else {
			Output::Warning("Lưu tệp tin cài đặt thất bại!");
		}
		return false;
	}
	...
	if (silent) {
		Output::Debug("Đã lưu cài đặt vào tệp tin {}", cfg_out.GetName());
	} else {
