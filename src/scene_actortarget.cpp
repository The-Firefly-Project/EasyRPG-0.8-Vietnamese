...
void Scene_ActorTarget::Start() {
	...
	if (use_item) {
		...
		if (item->type == lcf::rpg::Item::Type_special) {
			...
			if (!skill) {
				Output::Warning("Scene ActorTarget: Vật phẩm {} có chứa ID kĩ năng {} không hợp lệ", id, item->skill_id);
				Scene::Pop();
				return;
			}
	...
	if (Input::IsTriggered(Input::DECISION)) {
		...
		if (Main_Data::game_party->UseSkill(id, actor, target_window->GetActor())) {
			...
			if (animation) {
				...
				Output::Warning("UpdateSkill: Kĩ năng {} có hoạt ảnh {} không hợp lệ", id, skill->animation_id);
			}
		}
		else {
			Main_Data::game_system->SePlay(Main_Data::game_system->GetSystemSE(Main_Data::game_system->SFX_Buzzer));
