...
FilesystemView FileFinder::Save() {
	if (save_fs) {
		...
		if (!save_fs.IsFeatureSupported(Filesystem::Feature::Write)) {
			Output::Error("{} không phải là một nơi lưu hợp lệ (không thể ghi được)", GetFullFilesystemPath(save_fs));
		}
		return save_fs;
	}
	...
	if (!game_fs.IsFeatureSupported(Filesystem::Feature::Write)) {
		...
		for (;;) {
			...
			if (parent.IsFeatureSupported(Filesystem::Feature::Write)) {
				...
				if (!redir) {
					Output::Error("Thư mục lưu {} không hợp lệ", save_path);
				}
		...
		if (!redir) {
			Output::Error("Không có thư mục lưu phù hợp nào được tìm thấy");
		}
