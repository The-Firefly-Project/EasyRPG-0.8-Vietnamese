...
bool ImageBMP::ReadBMP(const uint8_t* data, unsigned len, bool transparent,
	...
	if (len < 64) {
		Output::Warning("Không phải là một tệp tin BMP hợp lệ.");
		return false;
	}
	auto* ptr = data;
	auto* e = data + len;
	...
	if (hdr.planes != 1) {
		Output::Warning("Planes của ảnh BMP không phải là 1.");
		return false;
	}

	if (hdr.depth != 8 && hdr.depth != 4) {
		Output::Warning("Độ sâu hình ảnh của ảnh BMP không được hỗ trợ: {} bit.", hdr.depth);
		return false;
	}

	if (hdr.compression) {
		Output::Warning("Ảnh BMP này đã được nén.");
		return false;
	}
	...
	if (!pixels) {
		Output::Warning("Lỗi phân bổ bộ đệm điểm ảnh BMP.");
		return false;
	}
