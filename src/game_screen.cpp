...
int Game_Screen::ShowBattleAnimation(int animation_id, int target_id, bool global, int start_frame) {
	...
	if (!anim) {
		Output::Warning("ShowBattleAnimation: ID hoạt ảnh chiến đấu {} không hợp lệ", animation_id);
		return 0;
	}

	auto* chara = Game_Character::GetCharacter(target_id, target_id);
	if (!chara) {
		Output::Warning("ShowBattleAnimation: ID sự kiện mục tiêu {} không hợp lệ", target_id);
