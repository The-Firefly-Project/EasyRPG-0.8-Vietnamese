...
FTFont::FTFont(Filesystem_Stream::InputStream is, int size, bool bold, bool italic)
	...
	if (!library) {
		if (FT_Init_FreeType(&library) != FT_Err_Ok) {
			Output::Error("Không thể tải thư viện FreeType");
			return;
		}
	}
...
Font::GlyphRet FTFont::vRenderShaped(char32_t glyph) const {
	...
	auto render_glyph = [&](auto flags, auto mode) {
		...
		if (FT_Render_Glyph(face->glyph, mode) != FT_Err_Ok) {
			Output::Error("Không thể kết xuất kí tự FreeType {:#x}", uint32_t(glyph));
		}
	};
...
/*
* NozakiYuu: Force the Player to use the Shinonome Gothic font that has been edited to support Vietnamese language by toaranotdev before.
* Original code:
