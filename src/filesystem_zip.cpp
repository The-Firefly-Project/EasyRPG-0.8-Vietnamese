		...
		if (ReadLocalHeader(zip_file, method, local_entry)) {
			if (central_entry->compressed_size != local_entry.compressed_size) {
				if (local_entry.compressed_size == 0) {
					...
					Output::Warning("ZipFS: Kích thước nén không khớp {}: {} != {}", path_normalized, central_entry->compressed_size, local_entry.compressed_size);
					return nullptr;
				}
			}
			...
			if (central_entry->uncompressed_size != local_entry.uncompressed_size) {
				if (local_entry.uncompressed_size == 0) {
					...
					Output::Warning("ZipFS: Kích thước không nén không khớp {}: {} != {}", path_normalized, central_entry->uncompressed_size, local_entry.uncompressed_size);
					return nullptr;
				}
			}

			if (local_entry.compressed_size == 0xffffffff || local_entry.uncompressed_size == 0xffffffff) {
				Output::Warning("ZipFS: Zip64 không được hỗ trợ {}", path_normalized);
				return nullptr;
			}
			...
			if (method == StorageMethod::Plain) {
				...
				if (zlib_error == Z_OK) {
					Output::Warning("ZipFS: zlib đã bị lỗi ở tệp tin {}: Có nhiều dữ liệu hơn (tệp tin nén đã bị hỏng?)", path_normalized);
					return nullptr;
				}
				else if (zlib_error != Z_STREAM_END) {
					Output::Warning("ZipFS: zlib đã bị lỗi ở tệp tin {}: {} ({})", path_normalized, zlib_error, zlib_stream.msg ? zlib_stream.msg : "Không có thông tin lỗi");
					return nullptr;
				}
				return new Filesystem_Stream::InputMemoryStreamBuf(std::move(dec_buf));
			} else {
				Output::Warning("ZipFS: {} có định dạng nén không được hỗ trợ. Chỉ hỗ trợ deflate", path_normalized);
				return nullptr;
			}
		}
	}
