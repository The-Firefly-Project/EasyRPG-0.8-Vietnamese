...
static bool ReadPNGWithReadFunction(png_voidp user_data, png_rw_ptr fn, bool transparent,
	...
	if (png_ptr == NULL) {
		Output::Warning("Không thể phân bổ cấu trúc PNG");
		return false;
	}

	png_info *info_ptr = png_create_info_struct(png_ptr);
	if (info_ptr == NULL) {
		Output::Warning("Không thể phân bổ cấu trúc thông tin PNG");
		return false;
	}
	...
	pixels = malloc(w * h * 4);
	if (!pixels) {
		Output::Warning("Lỗi phân bổ bộ đệm điểm ảnh PNG.");
		return false;
	}
...
static void ReadPalettedData(
	...
	if (!png_get_valid(png_ptr, info_ptr, PNG_INFO_PLTE)) {
		Output::Warning("Bảng màu PNG không có khối PLTE");
		return;
	}
...
bool ImagePNG::WritePNG(Filesystem_Stream::OutputStream& os, uint32_t width, uint32_t height, uint32_t* data) {
	...
	if (!write) {
		Output::Warning("Bitmap::WritePNG: lỗi ở lệnh png_create_write");
		return false;
	}
	...
	if (!info) {
		...
		Output::Warning("ImagePNG::WritePNG: lỗi ở lệnh png_create_info_struct");
		return false;
	}
	...
	if (setjmp(png_jmpbuf(write))) {
		png_destroy_write_struct(&write, &info);
		delete [] ptrs;
		Output::Warning("ImagePNG::WritePNG: không thể ghi tệp tin PNG");
		return false;
	}
