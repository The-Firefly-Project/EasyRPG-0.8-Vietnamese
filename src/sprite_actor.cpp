...
void Sprite_Actor::Update() {
	...
	if (anim_state > 0) {
		...
		if (Player::IsRPG2k3()) {
			...
			if (!anim) {
				Output::Warning("ID hoạt ảnh chiến đấu {} không hợp lệ", battler->GetBattleAnimationId());
				return;
			}

			const auto* ext = lcf::ReaderUtil::GetElement(anim->poses, anim_state);
			if (!ext) {
				Output::Warning("Hoạt ảnh {}: Trạng thái tiện ích mở rộng hoạt ảnh của nhân vật chiến đấu không hợp lệ {}", anim->ID, anim_state);
				return;
			}
...
void Sprite_Actor::SetAnimationState(int state, LoopState loop, int animation_id) {
	...
	if (battler->GetBattleAnimationId() > 0) {
		...
		if (ext->animation_type == lcf::rpg::BattlerAnimationPose::AnimType_battle) {
			...
			if (!battle_anim) {
				Output::Warning("ID hoạt ảnh chiến đấu {} không hợp lệ", animation_id);
				animation.reset();
			} else {
