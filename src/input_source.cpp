		...
		const std::string& replay_from_path)
{
	if (!replay_from_path.empty()) {
		...
		Output::Error("Không thể mở tệp để phát lại đầu vào: {}", path);
	}
...
{
	if (!log_file) {
		Output::Error("Lỗi đọc tệp tin nhật ký đầu vào {}", log_path);
		return;
	}
...
bool Input::Source::InitRecording(const std::string& record_to_path) {
	if (!record_to_path.empty()) {
		...
		if (!record_log) {
			Output::Error("Không thể mở tệp tin {} để ghi dữ liệu đầu vào: {}", path, strerror(errno));
			return false;
		}
