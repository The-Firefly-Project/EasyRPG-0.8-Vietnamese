...
std::string DynRpg::ParseVarArg(StringView func_name, dyn_arg_list args, int index, bool& parse_okay) {
	...
	if (index >= static_cast<int>(args.size())) {
		parse_okay = false;
		Output::Warning("{}: Vararg {} ngoài phạm vi", func_name, index);
		return "";
	}
		...
		if (chr == '$' && std::distance(text_index, end) > 1) {
			...
			if (n == '$') {
				...
				if (i + index < static_cast<int>(args.size())) {
					...
					// $-ref out of range
					parse_okay = false;
					Output::Warning("{}: Vararg $-ref {} ngoài phạm vi", func_name, i);
					return "";
				}
			...
			for (std::string::reverse_iterator it = tmp.rbegin(); it != tmp.rend(); ++it) {
				if (*it == 'N') {
					if (!Main_Data::game_actors->ActorExists(number)) {
						Output::Warning("{}: ID nhân vật {} trong {} không hợp lệ", function_name, number, token);
						return "";
					}
		...
		if (text_index == end) {
			switch (mode) {
				case ParseMode_Function:
					...
					if (function_name.empty()) {
						// empty function name
						Output::Warning("Trống tên hàm DynRPG");
						return "";
					}
					break;
			...
			switch (mode) {
				case ParseMode_Function:
					...
					break;
				case ParseMode_WaitForComma:
					Output::Warning("{}: \",\" được mong muốn, nhưng lại nhận được token", function_name);
					return "";
				case ParseMode_WaitForArg:
					if (chr == '"') {
...
void DynRpg::Load(int slot) {
	...
	if (!in) {
		Output::Warning("Không thể đọc bản lưu DynRPG: {}", filename);
	}
	...
	if (strncmp((char*)in_buffer.data(), "DYNSAVE1", 8) != 0) {
		Output::Warning("bản lưu DynRPG đã bị hỏng: {}", filename);
		return;
	}
...
void DynRpg::Save(int slot) {
	...
	if (!out) {
		Output::Warning("Không thể lưu bản lưu DynRPG: {}", filename);
		return;
	}
