	...
	if (!disable_rtp) {
		...
		if (is_rtp_asset) {
			if (is && game_has_full_package_flag && !warning_broken_rtp_game_shown && !is_audio_asset) {
				...
				Output::Warning("Trò chơi này nói rằng nó không cần RTP, nhưng thực ra lại đang sử dụng các tệp tin từ nó!");
			} else if (!is && !game_has_full_package_flag && !is_audio_asset) {
				std::string msg = "Không thể tìm thấy tệp tin: {}/{}. " +
								  std::string(search_paths.empty() ?
											  "Cài RTP {} để giải quyết cảnh báo này." : "RTP {} có vẻ như chưa được cài đặt đúng.");
