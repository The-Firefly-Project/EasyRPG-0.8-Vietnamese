...
void GenericAudio::SE_Play(std::unique_ptr<AudioSeCache> se, int volume, int pitch) {
	if (!se) {
		Output::Warning("SE_Play: Dữ liệu AudioSeCache không được xác định");
		return;
	}
	...
	Output::Debug("Không thể phát SE {}. Không còn kênh nào có sẵn", se->GetName());
}
...
bool GenericAudio::PlayOnChannel(BgmChannel& chan, Filesystem_Stream::InputStream filestream, int volume, int pitch, int fadein) {
	...
	if (!filestream) {
		Output::Warning("Tệp tin BGM không thể đọc được: {}", filestream.GetName());
		return false;
	}
	...
	if (chan.decoder && chan.decoder->Open(std::move(filestream))) {
		...
		Output::Warning("Không thể phát BGM {}. Tệp tin không được hỗ trợ", filestream.GetName());
	}
